...
From the above plots, data definitely does not look linear or gaussian. From this it is clear that trees are the best way forward. Linear or even Non-Linear Models would find it very difficult to classify
...
This is just as a starting point. However, glm does not run while running on a normal R file and hence, has not been included here. The main reason for selecting Trees over glm in this case is lesser restrictions (for example gaussian or linearity assumption) for the variables with Tree Models compared to Logistic Regression or Linear Discriminant Analysis. This was anyway not true when we looked at the qplot where we found that linearity assumption was not true.
We will use 3 tree models to see which one performs the best - Classification or Decision Tree, Random Forests and Bossting with Trees (gbm)
